...
async fn test_gemini_cost_calculation() {
    ...
    println!("Expected: prompt={} completion={} total={}", prompt_cost_cents, completion_cost_cents, total_expected_cost_cents);
    println!("Calculated: prompt={} completion={} total={}", calculated_prompt_cost, calculated_completion_cost, calculated_total);
    
    // Our implementation should produce reasonable costs
    assert!(calculated_total > 0, "Cost calculation should produce positive values");
    ...
    println!("Large scale: prompt={} completion={} total={}", large_prompt_cost, large_completion_cost, large_total);
...
async fn test_token_accumulation_logic() {
    ...
    for (prompt_tokens, completion_tokens) in updates {
        ...
        let cost_cents = ((prompt_tokens as f64 * 0.075 / 10.0) + (completion_tokens as f64 * 0.30 / 10.0)) as i32;
