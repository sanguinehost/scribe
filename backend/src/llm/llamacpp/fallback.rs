        ...
        Fut: std::future::Future<Output = Result<T, LocalLlmError>>,
    {
        ...
        match result {
            Ok(Ok(response)) => {
                ...
                debug!(
                    "Circuit breaker: request succeeded in {:?}",
                    start_time.elapsed()
                );
                Ok(response)
            }
            ...
            Err(_) => {
                ...
                self.on_failure(&mut metrics).await;
                warn!(
                    "Circuit breaker: request timed out after {:?}",
...
impl LlamaCppResilience {
        ...
        F: std::future::Future<Output = Result<T, LocalLlmError>>,
    {
            ...
            match result {
                ...
                Err(error) => {
                    ...
                    if attempt < self.retry_config.max_attempts {
                        ...
                        warn!(
                            "Resilience: attempt {} failed, retrying in {:?}: {}",
                            attempt, sleep_duration, error
                        );
        ...
        error!(
            "Resilience: all {} attempts failed, applying fallback strategy: {:?}",
        ...
        match self.fallback_strategy {
            ...
            FallbackStrategy::RemoteApi => Err(LocalLlmError::FallbackTriggered {
                reason: format!(
                    "Local LLM failed after {} attempts, should fallback to remote API",
                    self.retry_config.max_attempts
                ),
            }),
            FallbackStrategy::Queue => Err(LocalLlmError::FallbackTriggered {
                reason: "Request queued for later retry".to_string(),
            }),
            FallbackStrategy::Cache => Err(LocalLlmError::FallbackTriggered {
...
mod tests {
    ...
    async fn test_circuit_breaker_success() {
        ...
        let result = circuit_breaker
            .execute(|| async { Ok::<String, LocalLlmError>("success".to_string()) })
            .await;
    ...
    async fn test_circuit_breaker_failure() {
        ...
        let result = circuit_breaker
            .execute(|| async {
                Err::<String, LocalLlmError>(LocalLlmError::ServerUnavailable(
    ...
    async fn test_circuit_breaker_opens() {
        ...
        let result = circuit_breaker
            .execute(|| async { Ok::<String, LocalLlmError>("should be blocked".to_string()) })
            .await;
