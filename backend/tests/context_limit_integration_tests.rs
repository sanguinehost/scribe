...
mod context_limit_tests {
    ...
    async fn test_security_config_allows_1m_tokens() -> AnyhowResult<()> {
        ...
        println!(
            "✅ SecurityConfig supports {} tokens (>= 1M)",
            security_config.max_context_tokens
        );
        Ok(())
    }
    ...
    async fn test_context_configuration_ranges() -> AnyhowResult<()> {
        ...
        for limit in test_limits {
            ...
            assert!(
                limit >= 4096,
                "Context limit {} should be at least 4K",
                limit
            );
            assert!(
                limit <= 1048576,
                "Context limit {} should be at most 1M",
                limit
            );
            ...
            assert!(
                history_budget > 0,
                "History budget should be positive for limit {}",
                limit
            );
            assert!(
                rag_budget > 0,
    ...
    async fn test_llamacpp_default_context_size_reasonable() -> AnyhowResult<()> {
        ...
        println!(
            "✅ LlamaCpp default context size: {} tokens",
            config.context_size
        );
        Ok(())
    }
