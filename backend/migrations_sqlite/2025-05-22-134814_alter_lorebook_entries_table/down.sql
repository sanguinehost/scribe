...
-- Original: down.sql
-- Conversion date: 2025-10-19T11:00:19.548034
--
...
ADD COLUMN name TEXT, -- This was present in old schema, but not explicitly dropped if new design had 'entry_title'
...
-- Revert lorebook_id from TEXT to INTEGER
-- Drop the TEXT lorebook_id column
...
-- Re-add the foreign key constraint to the (now INTEGER) lorebooks.id
-- This assumes lorebooks.id has been reverted to INTEGER in its own down migration
...
ALTER TABLE lorebook_entries ADD COLUMN id INTEGER PRIMARY KEY AUTOINCREMENT;
