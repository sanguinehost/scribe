...
apply_container_permissions() {
    ...
    if [[ "$runtime" == "podman" ]]; then
        ...
        if command -v docker &> /dev/null; then
            ...
            docker run --rm -v "$POSTGRES_CERTS_DIR:/certs:Z" alpine:3.20 sh -c "
                chown -R 999:999 /certs
                chmod 644 /certs/cert.pem /certs/ca.pem 2>/dev/null || true
                chmod 600 /certs/key.pem
            "
        else
            log_warning "Docker not available - setting permissions with current user"
            log_warning "PostgreSQL may fail to start due to ownership requirements"
