<script lang="ts">
	...
	function generateOrderPreview() {
		...
		const amount = isYearly
			? selectedPlanFeatures.price_yearly!
			: selectedPlanFeatures.price_monthly;
		...
		orderPreview = {
			...
			savings_message: isYearly
				? selectedPlanFeatures.billing_features?.yearly?.savings_message
				: undefined,
			...
			<div class="space-y-8">
				...
				<div class="space-y-4 text-center">
					...
					<p class="mx-auto max-w-2xl text-lg text-muted-foreground">
						Unlock the full potential of character AI with our flexible subscription plans. Start
						your journey today.
					</p>
				</div>
		...
		{:else if checkoutStep === 'review'}
			...
			<div class="grid grid-cols-1 items-start gap-8 lg:grid-cols-2">
				...
				<div class="space-y-6">
					...
					<div class="space-y-4">
						...
						<p class="text-muted-foreground">
							You're about to subscribe to the <strong>{selectedPlanFeatures?.display_name}</strong>
							plan. Review the details and proceed to secure checkout.
						</p>
					</div>
					...
					<div class="space-y-4">
						...
						<p class="text-center text-xs text-muted-foreground">
							...
							purchase. Your subscription will be activated immediately after payment confirmation.
						</p>
					</div>
				</div>
				...
				<div class="lg:sticky lg:top-8">
					<OrderSummary {orderPreview} planFeatures={selectedPlanFeatures} {isLoading} />
				</div>
			</div>
		{:else if checkoutStep === 'processing'}
			...
			<div class="space-y-6 py-12 text-center">
				<div class="mx-auto h-16 w-16">
					<Loader class="h-full w-full animate-spin text-primary" />
				</div>
				<div class="space-y-2">
