...
mod character_editing_tests {
    ...
    async fn test_handle_character_edit_oneliner_success() {
        ...
        let mock_updated_char =
            mock_character_data_for_client(char_id, updated_char_name, Some("Updated Desc"));

        mock_http_client.update_character_result = Some(Arc::new(Ok(mock_updated_char.clone())));
    ...
    async fn test_handle_character_edit_wizard_success() {
        ...
        let original_char =
            mock_character_data_for_client(char_id, "Original Wizard", Some("Original Desc"));
        mock_http_client.get_character_result = Some(Arc::new(Ok(original_char.clone())));

        let updated_response_char = mock_character_data_for_client(
            char_id,
            "Wizard Updated Name",
            Some("Wizard Updated Desc"),
        );
        mock_http_client.update_character_result =
            Some(Arc::new(Ok(updated_response_char.clone())));
        ...
        mock_io_handler
            .expect_output("New Description (leave blank to keep current, type 'none' to clear):");
    ...
    async fn test_handle_character_edit_wizard_no_changes_made() {
        ...
        let original_char =
            mock_character_data_for_client(char_id, "Original NoChange", Some("NoChange Desc"));
        mock_http_client.get_character_result = Some(Arc::new(Ok(original_char.clone())));
        // update_character_result should not be called
    ...
    async fn test_handle_character_edit_oneliner_client_error() {
        ...
        mock_http_client.update_character_result = Some(Arc::new(Err(MockCliError::ApiError(
            "Failed to update character on server".to_string(),
        ))));
