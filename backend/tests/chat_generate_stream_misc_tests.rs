...
async fn generate_chat_response_streaming_reasoning_chunk() {
   ...
   let request = Request::builder()
    ...
    if all_session_messages.len() < 2 {
        ...
        assert!(all_session_messages.len() >= 1, 
        ...
        let has_user = all_session_messages.iter().any(|m| m.message_type == MessageRole::User);
        let has_assistant = all_session_messages.iter().any(|m| m.message_type == MessageRole::Assistant);
    ...
    if !all_session_messages.is_empty() {
        // Find a user message
        if let Some(user_msg) = all_session_messages.iter().find(|m| m.message_type == MessageRole::User) {
            // If it doesn't have a nonce, it's likely the manually inserted one
            if user_msg.content_nonce.is_none() {
                assert_eq!(String::from_utf8_lossy(&user_msg.content), "User message for reasoning chunk",
                ...
                if let Ok(decrypted_user_content_bytes) = scribe_backend::crypto::decrypt_gcm(
                    &user_msg.content,
                    nonce,
                    dek_for_assertion
                ) {
                    ...
                    assert_eq!(decrypted_user_content_str, "User message for reasoning chunk",
        ...
        if let Some(ai_msg) = all_session_messages.iter().find(|m| m.message_type == MessageRole::Assistant) {
            if let Some(nonce) = &ai_msg.content_nonce {
                if let Ok(decrypted_ai_content_bytes) = scribe_backend::crypto::decrypt_gcm(
                    &ai_msg.content,
                    nonce,
                    dek_for_assertion
                ) {
                    ...
                    assert_eq!(decrypted_ai_content_str, "Final answer. ",
