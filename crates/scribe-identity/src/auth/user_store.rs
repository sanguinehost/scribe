...
pub async fn create_user_in_db(
    ...
    let kek_salt = scribe_crypto::generate_salt().context("KEK salt generation failed")?;
    ...
    if let Some(provided_dek_ss) = plaintext_dek_opt {
        ...
        dek_to_encrypt = scribe_crypto::generate_dek().context("DEK generation failed")?;
    }

    let kek = scribe_crypto::derive_kek(&SecretString::from(password_str.to_string()), &kek_salt)
        .context("KEK derivation failed")?;

    let (encrypted_dek_bytes, dek_nonce_bytes) =
        scribe_crypto::encrypt_gcm(dek_to_encrypt.expose_secret(), &kek) // expose_secret() on SecretBox<Vec<u8>> gives &Vec<u8>
