<script lang="ts">
	...
	onMount(() => {
		...
		const isStatusBlock = /^(CURRENT STATE|INVENTORY|STATUS|STATS|CHARACTER|PARTY|LOCATION|HEALTH|EQUIPMENT)[\s\(].*:/mi.test(text) ||
		...
		const colorizedLines = lines.map(line => {
			...
			if (kvMatch) {
				...
				} else if (key.toLowerCase().includes('power') || key.toLowerCase().includes('attainment')) {
...
{#if inline}
<code bind:this={element} class={cn('whitespace-pre-wrap break-words text-sm', c)} {...props}>{@render children?.()}</code>
{:else}
<div class="not-prose flex flex-col mt-4">
<pre class="w-full overflow-x-auto rounded-xl border border-zinc-200 p-4 text-sm text-zinc-900 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-50"><code bind:this={element} class="whitespace-pre-wrap break-words" {...props}>{@render children?.()}</code></pre>
</div>
{/if}
