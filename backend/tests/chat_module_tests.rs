...
mod get_session_data_for_generation_tests {
    ...
    async fn setup_test_env(
        ...
        let _default_character_for_db_priming_if_needed =
            character_db_details.clone().unwrap_or_else(|| Character {
                id: character_id,
                user_id,
                ...
                post_history_instructions: Some(
                    "Char post history instructions".as_bytes().to_vec(),
                ),
        ...
        let mock_embedding_pipeline_for_test_setup =
            Arc::new(mock_embedding_pipeline_service_concrete.clone());
        ...
        .with_embedding_pipeline_service(Arc::new(mock_embedding_pipeline_service_concrete)
            as Arc<
    ...
    async fn test_recent_history_windowing_basic_fits_budget() {
        ...
        let inserted_user_id: Uuid = conn
            .interact(move |conn_insert| {
        ...
        let mut test_character_basic_fits =
        ...
        let test_session_basic_fits = NewChat {
            ...
            character_id: setup._character_id,
        ...
        for (plain_content_str, role_enum, tokens_opt, time_offset_secs) in
            message_definitions.iter()
        {
    ...
    async fn test_rag_lorebook_inclusion_fits_budget() {
        ...
        let new_user_for_rag_lore_test = NewUser {
            username: "testuser_rag_lore".to_string(),
            password_hash: "hash_rag_lore".to_string(),
            email: "raglore@example.com".to_string(),
            role: UserRole::User,
            account_status: AccountStatus::Active,
            kek_salt: "salt_rag_lore".to_string(),
            encrypted_dek: vec![2u8; 16],
            dek_nonce: vec![2u8; 12],
            encrypted_dek_by_recovery: None,
            recovery_kek_salt: None,
            recovery_dek_nonce: None,
        };
        let inserted_user_id_rag_lore: Uuid = conn
        ...
        // Insert Character
        let mut test_character_rag_lore =
            scribe_backend::models::characters::create_dummy_character();
        ...
        let (hist_content_bytes_for_db, hist_nonce_for_db) =
            if let Some(dek) = setup.user_dek.as_ref() {
                let (ciphertext, nonce) =
                    crypto::encrypt_gcm(history_msg_content.as_bytes(), dek.as_ref()).unwrap();
                (ciphertext, Some(nonce))
            } else {
    ...
    async fn test_rag_lorebook_exclusion_due_to_total_budget() {
        ...
        for i in 0.. {
            ...
            {
                let created_at =
                    chrono::Utc::now() + chrono::Duration::seconds(time_offset_base - i as i64);
                constructed_message_data_for_insertion.push((
                    long_hist_msg_content.to_string(),
                    MessageRole::User,
                    Some(tokens_per_long_hist_msg as i32),
                    created_at,
                ));
        ...
        for (plain_content_str, role_enum, tokens_opt, _created_at_val) in
            constructed_message_data_for_insertion.iter()
        {
            let (content_bytes_for_db, nonce_for_db) = if let Some(dek) = setup.user_dek.as_ref() {
                let (ciphertext, nonce) =
                    crypto::encrypt_gcm(plain_content_str.as_bytes(), dek.as_ref()).unwrap();
