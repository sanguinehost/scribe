...
pub type DbConn =
    diesel::r2d2::PooledConnection<diesel::r2d2::ConnectionManager<crate::DbConnection>>;
...
pub trait SqliteInteractExt {
    async fn interact<F, T>(&mut self, f: F) -> Result<T, DatabaseError>
    where
        F: FnOnce(&mut crate::DbConnection) -> T + Send + 'static,
        T: Send + 'static;
}

#[cfg(feature = "sqlite-backend")]
impl SqliteInteractExt
    for diesel::r2d2::PooledConnection<diesel::r2d2::ConnectionManager<crate::DbConnection>>
{
    async fn interact<F, T>(&mut self, f: F) -> Result<T, DatabaseError>
    where
        F: FnOnce(&mut crate::DbConnection) -> T + Send + 'static,
...
#[cfg(feature = "postgres-backend")]
pub async fn get_conn(pool: &DbPool) -> Result<DbConn, DatabaseError> {
    pool.get()
        .await
        .map_err(|e| DatabaseError::DatabaseQueryError(format!("Failed to get connection: {}", e)))
}
...
pub async fn get_conn(pool: &DbPool) -> Result<DbConn, DatabaseError> {
    ...
    .await
    .map_err(|e| DatabaseError::DatabaseQueryError(format!("Task join error: {}", e)))?
}
...
///         .map_err(|e| DatabaseError::DatabaseQueryError(e.to_string()))
/// }).await?;
/// ```
#[cfg(feature = "postgres-backend")]
pub async fn with_conn<F, T>(pool: &DbPool, f: F) -> Result<T, DatabaseError>
where
    F: FnOnce(&mut crate::DbConnection) -> Result<T, DatabaseError> + Send + 'static,
    T: Send + 'static,
{
    let conn = pool
        .get()
    ...
    conn.interact(f)
        .await
        .map_err(|e| DatabaseError::DatabaseQueryError(format!("Interaction error: {}", e)))?
}
...
#[cfg(feature = "postgres-backend")]
pub async fn with_conn_immediate<F, T>(pool: &DbPool, f: F) -> Result<T, DatabaseError>
where
    F: FnOnce(&mut crate::DbConnection) -> Result<T, DatabaseError> + Send + 'static,
    T: Send + 'static,
{
    ...
    T: Send + 'static,
{
    ...
    tokio::task::spawn_blocking(move || {
        let mut conn = pool.get().map_err(|e| {
            DatabaseError::DatabaseQueryError(format!("Failed to get connection: {}", e))
        })?;
        f(&mut conn)
    })
    .await
    ...
    tokio::task::spawn_blocking(move || {
            ...
            match begin_result {
                ...
                Err(e) => {
                    ...
                    if attempts >= max_attempts {
                        return Err(DatabaseError::DatabaseQueryError(format!(
        ...
        if result.is_ok() {
            diesel::sql_query("COMMIT")
                .execute(&mut conn)
                .map_err(|e| {
                    DatabaseError::DatabaseQueryError(format!(
