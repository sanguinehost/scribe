...
async fn test_qdrant_upsert_points() -> Result<(), AnyhowError> {
    ...
    for (i, vector) in test_vectors.iter().enumerate() {
        ...
        let point = create_qdrant_point(
            id.into(),
            vector.clone(),
            Some(scribe_backend::db::DbJson::from(payload)),
        )?;
        points.push(point);
    }
...
async fn test_qdrant_update_existing_point() -> Result<(), AnyhowError> {
    ...
    let initial_point = create_qdrant_point(
        point_id.into(),
        vector.clone(),
        Some(scribe_backend::db::DbJson::from(initial_payload)),
    )?;
    ...
    let updated_point = create_qdrant_point(
        point_id.into(),
        vector.clone(),
        Some(scribe_backend::db::DbJson::from(updated_payload)),
    )?;
...
async fn test_qdrant_retrieve_points() -> Result<(), AnyhowError> {
    ...
    let point_1 = create_qdrant_point(
        ...
        Some(scribe_backend::db::DbJson::from(payload_1)),
    )?;
    let point_2 = create_qdrant_point(
        point_id_2.into(),
        test_vectors[1].clone(),
        Some(scribe_backend::db::DbJson::from(payload_2)),
    )?;
...
async fn test_qdrant_trait_methods() -> Result<(), AnyhowError> {
    ...
    let point_trait = create_qdrant_point(
        point_id_trait.into(),
        test_vectors[0].clone(),
        Some(scribe_backend::db::DbJson::from(payload_trait)),
    )?;
