...
mod chat_override_tests {
    ...
    async fn test_handle_chat_edit_character_oneliner_success() {
        ...
        assert!(result.is_ok());
        mock_io_handler
            .expect_output("Setting chat character override from command line arguments...");
    ...
    async fn test_handle_chat_edit_character_wizard_success() {
        ...
        let mock_effective_char = mock_character_data_for_client(
            original_character_id,
            "Original Char",
            Some("Original Desc"),
        );
        mock_http_client.get_effective_character_for_chat_result =
            Some(Arc::new(Ok(mock_effective_char.clone())));
        ...
        mock_http_client.set_chat_character_override_result =
            Some(Arc::new(Ok(mock_override_response)));
        ...
        mock_io_handler.expect_output(&format!(
            "Current value for '{}': {}",
            field_to_edit,
            mock_effective_char.description.as_deref().unwrap_or("N/A")
        ));
        mock_io_handler.expect_output(&format!(
    ...
    async fn test_handle_chat_edit_character_wizard_cancel_submission() {
        ...
        let mock_session_details = mock_chat_session_details(session_id, original_character_id);
        mock_http_client.get_chat_session_result = Some(Arc::new(Ok(mock_session_details)));
        let mock_effective_char =
            mock_character_data_for_client(original_character_id, "Original", Some("Desc"));
        mock_http_client.get_effective_character_for_chat_result =
            Some(Arc::new(Ok(mock_effective_char)));
