...
fn insert_test_character(
    ...
    let (encrypted_description, description_nonce) =
        crypto::encrypt_gcm(default_description.as_bytes(), &dek.0) // Use inner dek
    ...
    let (encrypted_personality, personality_nonce) =
        crypto::encrypt_gcm(default_personality.as_bytes(), &dek.0) // Use inner dek
...
mod tests {
    ...
    async fn test_generate_character() -> Result<(), anyhow::Error> {
        ...
        assert_eq!(
            found_character.id, character.id,
            "Character ID should match"
        );
        assert_eq!(
            found_character.user_id, user.id,
            "Character user_id should match the test user"
        );
        assert_eq!(
            found_character.name, "Generated Wizard Character",
            "Character name should match"
        );
        assert_eq!(
            found_character.spec, "chara_card_v3",
            "Character spec should match"
        );
        assert_eq!(
            found_character.spec_version, "1.0",
            "Character spec_version should match"
        );
    ...
    async fn test_delete_character_success() -> Result<(), anyhow::Error> {
        ...
        let user_in_db = db::create_test_user(&pool, username.clone(), password.clone())
            .await
        ...
        let character = db::create_test_character(&pool, user_in_db.id, character_name.clone())
            .await
        ...
        let conn = pool
            .get()
            .await
            .context("Failed to get DB connection for verification")?;
        ...
        let character_check_result = conn
            .interact(move |conn_block| {
                use diesel::prelude::*;
                use scribe_backend::schema::characters::dsl::*;
        ...
        if let Some(found_character) = character_check_result {
            ...
            assert_eq!(
                found_character.user_id, user_in_db.id,
        ...
        let auth_check_response = test_app
            .router
            .clone()
        ...
        let conn_verify = pool
            .get()
            .await
            .context("Failed to get DB connection for verification")?;
        let result = conn_verify
            .interact(move |conn_block| {
                use scribe_backend::schema::characters::dsl::*;
                let exists =
                    diesel::select(diesel::dsl::exists(characters.filter(id.eq(character.id))))
                        .get_result::<bool>(conn_block);

                match exists {
                    Ok(true) => tracing::error!("Character with ID {} exists in DB", character.id),
                    Ok(false) => {
                        tracing::error!("Character with ID {} does NOT exist in DB", character.id)
                    }
        ...
        let verify_conn = pool
            .get()
            .await
            .context("Failed to get DB connection for deletion verification")?;
        let character_id_copy = character.id; // For use in closure

        let character_verify = verify_conn
            .interact(move |conn_block| {
                use diesel::prelude::*;
                use scribe_backend::schema::characters::dsl::*;
            ...
            .interact(move |conn_block| {
                diesel::delete(
                    scribe_backend::schema::users::table
                        .filter(scribe_backend::schema::users::id.eq(user_in_db.id)),
                )
                .execute(conn_block)
            })
            .await;
    ...
    async fn test_upload_missing_file_field() -> Result<(), anyhow::Error> {
        ...
        let mut body = Vec::new();
        body.extend_from_slice(format!("--{}\r\n", boundary).as_bytes());
        body.extend_from_slice(b"Content-Disposition: form-data; name=\"other_field\"\r\n\r\n");
        ...
        tracing::info!(
            "Testing that multipart data is correctly structured but missing character_card field"
        );
        ...
        let body_text =
            String::from_utf8(body_bytes.to_vec()).context("Failed to convert body to string")?;
    ...
    async fn test_get_character_forbidden() -> Result<(), anyhow::Error> {
        ...
        let character_verify = conn_verify
            .interact(move |conn_block| {
                use diesel::prelude::*;
                use scribe_backend::schema::characters::dsl::*;
        ...
        assert!(
            character_verify.is_some(),
            "User A should be able to access the character"
        );
    ...
    async fn test_delete_nonexistent_character() -> Result<(), anyhow::Error> {
        ...
        assert_eq!(
            rows_affected, 0,
            "No rows should be affected when deleting a non-existent character"
        );
    ...
    async fn test_delete_character_forbidden() -> Result<(), anyhow::Error> {
        ...
        let character_exists = conn_verify
            .interact(move |conn_block| {
                use diesel::prelude::*;
                use scribe_backend::schema::characters::dsl::*;
        ...
        assert!(
            character_exists,
            "Character should exist before attempting forbidden delete"
        );
        ...
        assert_eq!(
            rows_affected, 0,
            "No rows should be affected when User B tries to delete User A's character"
        );
        ...
        let conn_check = pool
            .get()
            .await
            .context("Failed to get DB connection for final check")?;
        let char_id_final = character_a.id;
        let user_a_id_final = user_a.id;

        let still_exists = conn_check
            .interact(move |conn_block| {
                use diesel::prelude::*;
                use scribe_backend::schema::characters::dsl::*;
        ...
        tracing::info!(
            "Successfully verified character still exists after forbidden delete attempt"
        );
    ...
    async fn test_list_characters_success() -> Result<(), anyhow::Error> {
        ...
        assert!(
            sorted_chars[0].description.is_some(),
            "First character should have encrypted description data"
        );
        assert!(
            sorted_chars[0].description_nonce.is_some(),
            "First character should have description nonce"
        );
        ...
        assert!(
            sorted_chars[1].description.is_some(),
            "Second character should have encrypted description data"
        );
        assert!(
            sorted_chars[1].description_nonce.is_some(),
            "Second character should have description nonce"
        );
