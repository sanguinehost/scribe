...
use scribe_backend::models::user_personas::{UserPersonaDataForClient, CreateUserPersonaDto}; // Changed UserPersona to UserPersonaDataForClient
use scribe_backend::test_helpers::{spawn_app, TestDataGuard, db, login_user_via_api};
...
async fn get_me_includes_default_persona_id() {
    ...
    assert!(auth_response.default_persona_id.is_none(), "Initially, default_persona_id should be None");
    ...
    let auth_response: AuthResponse = response.json().await.unwrap();
    assert_eq!(auth_response.user_id, user_db.id);
    assert!(auth_response.default_persona_id.is_none(), "After clearing, default_persona_id should be None");
