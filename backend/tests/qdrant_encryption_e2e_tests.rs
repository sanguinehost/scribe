...
use scribe_backend::{
    ...
    services::{
        ...
        embeddings::{
            EmbeddingPipelineService, EmbeddingPipelineServiceTrait, LorebookEntryParams,
        },
    },
...
async fn test_lorebook_entry_encryption_in_qdrant() -> Result<()> {
    ...
    assert!(
        !scroll_result.is_empty(),
        "Expected to find lorebook entries in Qdrant"
    );
    ...
    for point in &scroll_result {
        ...
        assert!(
            payload_json.contains_key("encrypted_chunk_text"),
            "Should have encrypted_chunk_text field"
        );
        assert!(
            payload_json.contains_key("chunk_text_nonce"),
            "Should have chunk_text_nonce field"
        );
        ...
        if let Some(chunk_text) = payload_json.get("chunk_text") {
            if let Some(text_str) = chunk_text.as_str() {
                assert_eq!(
                    text_str, "[encrypted]",
                    "Plaintext field should contain [encrypted] placeholder"
                );
                assert!(
                    !text_str.contains("dragon"),
                    "Should not contain actual content in plaintext field"
                );
            }
        }
    }
...
async fn test_chronicle_event_encryption_in_qdrant() -> Result<()> {
    ...
    let conn = test_app.db_pool.get().await?;
    let inserted_chronicle: scribe_backend::models::chronicle::PlayerChronicle = conn
    ...
    let conn = test_app.db_pool.get().await?;
    let inserted_event: scribe_backend::models::chronicle_event::ChronicleEvent = conn
        .interact(move |conn_sync| {
    ...
    assert!(
        !scroll_result.is_empty(),
        "Expected to find chronicle event in Qdrant"
    );
...
async fn test_encryption_isolation_between_users() -> Result<()> {
    ...
    assert!(
        ...
        "Should not be able to decrypt User A's data with User B's key"
    );
    ...
    assert_eq!(
        decrypted_content, content_a,
        "User A should be able to decrypt their own content"
    );
