...
<Dialog bind:open>
	<DialogContent>
		<DialogHeader>
			...
			<DialogDescription>
				Choose the export format for your lorebook
			</DialogDescription>
		</DialogHeader>
		
		<div class="space-y-3 py-4">
			<button
				class="w-full text-left p-4 rounded-lg border transition-colors {selectedFormat === 'scribe_minimal' ? 'border-primary bg-primary/5' : 'border-border hover:bg-accent'}"
			...
			<button
				class="w-full text-left p-4 rounded-lg border transition-colors {selectedFormat === 'silly_tavern_full' ? 'border-primary bg-primary/5' : 'border-border hover:bg-accent'}"
				onclick={() => selectFormat('silly_tavern_full')}
			>
				<div class="space-y-1">
					<h4 class="text-sm font-medium">SillyTavern (Full Format)</h4>
					<p class="text-sm text-muted-foreground">
						Complete format with all metadata including insertion order, placement hints, and probability settings.
					</p>
				</div>
			</button>
		</div>
