...
pub async fn handle_chat_edit_character_oneliner<H: IoHandler, C: HttpClient>(
    ...
    let session_id = args.session_id.ok_or_else(|| {
        CliError::InputError("Missing --session-id argument for chat character override.".to_string())
    })?;
    ...
    let allowed_fields = ["description", "personality", "first_mes", "system_prompt", "scenario"]; // Add more as supported
...
pub async fn handle_chat_edit_character_wizard<H: IoHandler, C: HttpClient>(
    ...
    let chat_session = client.get_chat_session(session_id).await.map_err(|e| {
        io_handler.write_line(&format!("Failed to fetch chat session {}: {}", session_id, e)).ok();
        e
    })?;
    let original_character_id = chat_session.character_id;

    io_handler.write_line(&format!("Fetching effective character data for session {} (Original Character ID: {})...", session_id, original_character_id))?;
    
    let effective_char = client.get_effective_character_for_chat(original_character_id, session_id).await.map_err(|e|{
        io_handler.write_line(&format!("Failed to fetch effective character data: {}", e)).ok();
        e
    })?;
    ...
    io_handler.write_line(&format!("  Description: {}", effective_char.description.as_deref().unwrap_or("N/A")))?;
    io_handler.write_line(&format!("  Personality: {}", effective_char.personality.as_deref().unwrap_or("N/A")))?;
    io_handler.write_line(&format!("  First Message: {}", effective_char.first_mes.as_deref().unwrap_or("N/A")))?;
    io_handler.write_line(&format!("  System Prompt: {}", effective_char.system_prompt.as_deref().unwrap_or("N/A")))?;
    io_handler.write_line(&format!("  Scenario: {}", effective_char.scenario.as_deref().unwrap_or("N/A")))?;
    ...
    let overridable_fields = vec!["description", "personality", "first_mes", "system_prompt", "scenario"];
    ...
    let field_choice: usize = field_choice_str.parse().map_err(|_| CliError::InputError("Invalid number.".to_string()))?;
    ...
    let current_value = match field_to_override.as_str() {
        "description" => effective_char.description.as_deref().unwrap_or("").to_string(),
        "personality" => effective_char.personality.as_deref().unwrap_or("").to_string(),
        "first_mes" => effective_char.first_mes.as_deref().unwrap_or("").to_string(),
        "system_prompt" => effective_char.system_prompt.as_deref().unwrap_or("").to_string(),
        "scenario" => effective_char.scenario.as_deref().unwrap_or("").to_string(),
        _ => "".to_string(), // Should not happen due to check above
    };

    io_handler.write_line(&format!("Current value for '{}': {}", field_to_override, if current_value.is_empty() { "N/A" } else { &current_value }))?;
    let new_value = io_handler.read_line(&format!("Enter new value for '{}' (leave blank to clear override, if supported by backend):", field_to_override))?;

    if crate::handlers::characters::prompt_yes_no(io_handler, &format!("\nApply override for '{}' to '{}'? (Y/n)", field_to_override, new_value))? {
        match client.set_chat_character_override(session_id, field_to_override.clone(), new_value.clone()).await {
            Ok(response) => {
                io_handler.write_line(&format!(
                    "Successfully applied override for field '{}' in chat session {}.",
                    response.field_name, response.session_id
