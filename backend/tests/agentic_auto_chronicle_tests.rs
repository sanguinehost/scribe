...
mod agentic_chronicle_tests {
    ...
    async fn test_agentic_system_auto_creates_chronicle_for_significant_events() {
        ...
        assert!(
            !events.is_empty(),
            "Should have extracted events to the chronicle"
        );

        let event = &events[0];
        assert_eq!(
            event.get_source().unwrap(),
            EventSource::AiExtracted,
            "Events should be AI-extracted"
        );

        // Decrypt the summary to check its content
        let decrypted_summary = event
            .get_decrypted_summary(&session_dek.0)
            .unwrap_or_else(|_| event.summary.clone()); // Fallback to plaintext if decryption fails
        assert!(
            decrypted_summary.contains("Alex") || decrypted_summary.contains("adventure"),
            "Event summary should be contextually relevant: {}",
            decrypted_summary
        );
    }
    ...
    async fn test_agentic_system_adds_to_existing_chronicle() {
        ...
        let existing_chronicle = chronicle_service
        ...
        let events = chronicle_service
            .get_chronicle_events(user_id, existing_chronicle.id, Default::default())
            .await
            .unwrap();
        assert!(
            !events.is_empty(),
            "Should have added events to existing chronicle"
        );
        ...
        // Decrypt the summary to check its content
        let decrypted_summary = latest_event
            .get_decrypted_summary(&session_dek.0)
            .unwrap_or_else(|_| latest_event.summary.clone()); // Fallback to plaintext if decryption fails
        assert!(
            decrypted_summary.contains("Professor Willowshade")
                || decrypted_summary.contains("meet"),
            "Event should document the character meeting: {}",
            decrypted_summary
        );

        // Verify no new chronicle was created (should still only have 1)
        let all_chronicles = chronicle_service
        ...
        assert_eq!(
            all_chronicles.len(),
            1,
            "Should not have created a new chronicle"
        );
        assert_eq!(
            all_chronicles[0].id, existing_chronicle.id,
            "Should have used existing chronicle"
        );
    }
    ...
    async fn test_agentic_system_handles_errors_gracefully() {
        ...
        assert!(
            result.is_err(),
            "Should return error for invalid AI responses"
        );
        ...
        assert!(
            error.to_string().contains("JSON")
                ...
                || error.to_string().contains("AI")
                || error
                    .to_string()
                    .contains("Failed to parse structured response"),
            "Error should be related to JSON parsing or AI client: {}",
            error
        );
        ...
        assert!(
            chronicles.is_empty(),
            "Should not have created chronicles when errors occur"
        );
    }
}
