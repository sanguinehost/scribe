...
impl HybridTokenCounter {
            ...
            CountingMode::ApiOnly => {
                ...
                if let Some(client) = &self.api_client {
                    ...
                    Err(AppError::ConfigError(
                        "API client not configured for token counting".to_string(),
                    ))
                }
            }
            ...
            CountingMode::HybridPreferApi => {
                ...
                if let Some(client) = &self.api_client {
                    match client.count_tokens(text, model_name).await {
                        ...
                        Err(e) => {
                            warn!(
                                "API token counting failed, falling back to local estimation: {}",
                                e
                            );
                            self.tokenizer.estimate_text_tokens(text)
                        }
    ...
    pub async fn count_tokens_multimodal(
        ...
        if let Some(client) = &self.api_client {
            ...
            if mode == CountingMode::ApiOnly
                && (video_duration.is_some() || audio_duration.is_some())
            {
...
mod tests {
    ...
    async fn test_count_multimodal_local_only() {
        ...
        assert_eq!(
            estimate.total,
            estimate.text + estimate.video + estimate.audio
        );
    ...
    async fn test_count_chat_local_only() {
        ...
        let single_estimate = counter
            .count_tokens("Hi my name is Bob", CountingMode::LocalOnly, None)
            .await
