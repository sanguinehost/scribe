...
impl EncryptionService {
    ...
    pub async fn encrypt(&self, plaintext: &str, key: &[u8]) -> Result<(Vec<u8>, Vec<u8>), AppError> {
    ...
    pub async fn decrypt(&self, ciphertext_and_tag: &[u8], nonce_bytes: &[u8], key: &[u8]) -> Result<Vec<u8>, AppError> {
        if key.len() != aead::AES_256_GCM.key_len() {
        ...
        match crate::crypto::decrypt_gcm(ciphertext_and_tag, nonce_bytes, &key_secret_box) {
            Ok(decrypted_secret_box_vec) => {
                Ok(decrypted_secret_box_vec.expose_secret().to_vec())
            }
            Err(e) => {
...
mod tests {
    ...
    async fn test_encrypt_decrypt_round_trip() {
        ...
        let decrypted_data_bytes = service.decrypt(&encrypted_data, &nonce, TEST_KEY).await.unwrap();
    ...
    async fn test_encrypt_empty_string() {
        ...
        assert_eq!(plaintext, decrypted_data);
        assert!(!encrypted_data.is_empty(), "Ciphertext (even for empty plaintext) should not be empty due to AEAD tag");
