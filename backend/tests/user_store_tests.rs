...
mod user_store_tests {
    ...
    async fn test_basic_credential_verification(
        ...
        let (username, _email, password_secret) = generate_test_user_data();

        let created_user = create_test_user_for_verification(
            pool,
            username.clone(),
            _email.clone(), // This is ignored by create_test_user
            password_secret.clone(),
        )
        .await?;
        guard.add_user(created_user.id);

        // Use the actual email that was stored in the database
        let actual_email = &created_user.email;
    ...
    async fn create_test_user_for_verification(
        pool: &scribe_backend::state::DbPool,
        username: String,
        _email: String, // email is generated by create_test_user
        password: SecretString,
    ) -> Result<scribe_backend::models::users::User, Box<dyn std::error::Error>> {
        // Use the test helper that creates already-verified users
        scribe_backend::test_helpers::db::create_test_user(
            pool,
            username,
            password.expose_secret().to_string(),
        )
        .await
        .map_err(|e| format!("Failed to create test user: {}", e).into())
    }
