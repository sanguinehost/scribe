...
describe('Raw Prompt Modal Scrolling Behavior', () => {
	...
	it('should have overflow-y-auto class on scroll container', () => {
		...
		content.className = 'p-4 text-xs leading-relaxed text-foreground/90 whitespace-pre-wrap break-words font-mono';
	...
	it('should show visible scrollbar on overflow in browser environment', () => {
		...
		content.textContent = Array(500)
			.fill(null)
			.map((_, i) => `Line ${i + 1}: This is a very long line of text that represents part of a raw prompt sent to the AI model. `.repeat(3))
			.join('\n');
		...
		Object.defineProperty(scrollContainer, 'scrollTop', {
			get: () => scrollTop,
			set: (value) => { scrollTop = Math.max(0, Math.min(value, scrollContainer.scrollHeight - scrollContainer.clientHeight)); },
			configurable: true
		});
