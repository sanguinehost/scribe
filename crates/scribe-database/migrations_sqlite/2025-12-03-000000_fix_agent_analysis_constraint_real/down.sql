-- SQLite Migration
-- Revert fix for agent_context_analysis constraint (restore strict unique constraint)

-- 1. Create table WITH the strict UNIQUE(chat_session_id, analysis_type) constraint
CREATE TABLE new_agent_context_analysis (
    ...
    -- RESTORE THE PROBLEMATIC CONSTRAINT
    UNIQUE(chat_session_id, analysis_type)
);

-- 2. Copy data from current table to new table
-- Note: This might fail if there are violating rows, but that's expected for a down migration if we fixed the data
...
-- The unique constraint per message (which was added in the fix attempt migration)
