...
async fn main() -> Result<()> {
    ...
    let (user, session_dek) = authenticate_user(&state, &args.username, &args.password)
        .await
    ...
    info!(
        "Processing {} chronicle events with max {} concurrent tasks",
        total_events, max_concurrent
    );
        ...
        let tasks: Vec<_> = chunk
            ...
            .map(|(idx_in_chunk, event)| {
                ...
                tokio::spawn(async move {
                    ...
                    {
                        error!(
                            "Failed to delete existing embeddings for event {}: {}",
                            event.id, e
                        );
...
async fn authenticate_user(
    ...
    let session_dek_bytes = crypto::decrypt_gcm(&user.encrypted_dek, &user.dek_nonce, &kek)
