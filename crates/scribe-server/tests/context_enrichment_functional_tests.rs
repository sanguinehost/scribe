...
async fn test_context_enrichment_complete_workflow_preprocessing() {
    ...
    let messages = vec![
        (
            "user".to_string(),
            "Tell me about Aragorn's sword".to_string(),
        ),
        (
            "assistant".to_string(),
            "Aragorn wielded Andúril, the Flame of the West".to_string(),
        ),
        (
            "user".to_string(),
            "What happened when he went to Minas Tirith?".to_string(),
        ),
    ];
    ...
    match result {
        Ok(enrichment_result) => {
            ...
            assert!(
                !enrichment_result.planned_searches.is_empty(),
                "Should have planned searches"
            );
            assert!(
                enrichment_result.planned_searches.len() >= 1,
                "Should have at least one search"
            );

            // Verify execution log
            assert!(
                !enrichment_result.execution_log.steps.is_empty(),
                "Execution log should have steps"
            );
            assert!(
                enrichment_result.execution_log.steps.len() >= 1,
                "Should have at least planning step"
            );
        ...
        Err(e) => {
            ...
            assert!(
                error_str.contains("ai")
                    || error_str.contains("gemini")
                    || error_str.contains("json")
                    || error_str.contains("model"),
                "Error should be related to AI/model issues: {}",
                e
            );
        }
    }
    ...
    match result {
        Ok(enrichment_result) => {
            ...
            assert_eq!(
                enrichment_result.mode,
                EnrichmentMode::PostProcessing,
                "Mode should be post-processing"
            );
            ...
            let step_types: Vec<String> = enrichment_result
                .execution_log
                .steps
                .iter()
                ...
                .collect();
            assert!(
                step_types.contains(&"planning".to_string()),
                "Should have planning step"
            );
        }
        Err(e) => {
            info!(
                "Post-processing enrichment failed (acceptable with mock AI): {}",
                e
            );
            let error_str = e.to_string().to_lowercase();
            assert!(
...
async fn test_context_enrichment_search_types() {
    ...
    let messages = vec![
        (
            "user".to_string(),
            "Remember when we talked about the ancient library in our chronicle?".to_string(),
        ),
        (
            "assistant".to_string(),
            "Yes, the Great Library of Alexandria in your world.".to_string(),
        ),
        (
            "user".to_string(),
            "I want to add more details about it from my lorebook".to_string(),
        ),
    ];
    ...
    match result {
        Ok(enrichment_result) => {
            ...
            let search_types: Vec<String> = enrichment_result
                .planned_searches
                .iter()
...
async fn test_context_enrichment_analysis_storage() {
    ...
    let messages = vec![
        (
            "user".to_string(),
            "Let's talk about dragons in our fantasy world".to_string(),
        ),
        (
            "assistant".to_string(),
            "Dragons are ancient and wise creatures in your realm".to_string(),
        ),
    ];
    ...
    match result {
        Ok(enrichment_result) => {
            ...
            assert_eq!(
                stored_analysis.chat_session_id, session_id,
                "Session ID should match"
            );
            assert!(
                stored_analysis.agent_reasoning.is_some(),
                "Reasoning should be stored"
            );
            assert!(
                stored_analysis.execution_log.is_some(),
                "Execution log should be stored"
            );
            assert!(
                stored_analysis.retrieved_context.is_some(),
                "Context should be stored"
            );
            assert!(
                stored_analysis.analysis_summary.is_some(),
                "Summary should be stored"
            );

            // Verify encryption nonces are present (indicating data is encrypted)
            assert!(
                stored_analysis.agent_reasoning_nonce.is_some(),
                "Reasoning nonce should be present"
            );
            assert!(
                stored_analysis.execution_log_nonce.is_some(),
                "Log nonce should be present"
            );
            assert!(
                stored_analysis.retrieved_context_nonce.is_some(),
                "Context nonce should be present"
            );
            assert!(
                stored_analysis.analysis_summary_nonce.is_some(),
                "Summary nonce should be present"
            );

            // Verify metadata
            assert_eq!(
                stored_analysis.total_tokens_used,
                Some(enrichment_result.total_tokens_used as i32),
                "Token count should match"
            );
            assert_eq!(
                stored_analysis.execution_time_ms,
                Some(enrichment_result.execution_time_ms as i32),
                "Execution time should match"
            );
            assert_eq!(
                stored_analysis.model_used,
        ...
        Err(e) => {
            ...
            let conn = test_app
                ...
                .await
                .expect("Failed to get connection");
            let stored_analysis_result = conn
                .interact(move |conn| {
                    use scribe_backend::models::AgentContextAnalysis;
                    use scribe_backend::schema::agent_context_analysis::dsl::*;
            ...
            assert!(
                stored_analysis_result.is_none(),
                "No analysis should be stored on enrichment failure"
            );
        }
    }
...
async fn test_context_enrichment_message_patterns() {
    ...
    let test_cases = vec![
        (
            ...
            vec![(
                "user".to_string(),
                "Tell me about magic systems".to_string(),
            )],
        ),
        (
            "Long conversation",
            vec![
                (
                    "user".to_string(),
                    "I want to create a character".to_string(),
                ),
                (
                    "assistant".to_string(),
                    "What kind of character interests you?".to_string(),
                ),
                (
                    "user".to_string(),
                    "A wizard from the mountains".to_string(),
                ),
                (
                    "assistant".to_string(),
                    "Mountain wizards often study earth magic".to_string(),
                ),
                (
                    "user".to_string(),
                    "Tell me more about earth magic traditions".to_string(),
                ),
                (
                    "assistant".to_string(),
                    "Earth magic involves stone shaping and crystal work".to_string(),
                ),
                (
                    "user".to_string(),
                    "What about their training rituals?".to_string(),
                ),
            ],
        ),
        (
            ...
            vec![
                (
                    "user".to_string(),
                    "Kael traveled to Silverwind City".to_string(),
                ),
                (
                    "assistant".to_string(),
                    "Silverwind City is known for its floating towers".to_string(),
                ),
                (
                    "user".to_string(),
                    "Did Kael meet anyone there?".to_string(),
                ),
            ],
        ),
    ];
            ...
            Err(e) => {
                ...
                assert!(
                    error_str.contains("ai")
                        || error_str.contains("gemini")
                        || error_str.contains("json"),
                    "Error for {} should be AI-related: {}",
                    test_name,
                    e
                );
            }
