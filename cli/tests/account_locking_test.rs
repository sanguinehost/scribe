...
async fn test_account_locking() {
    ...
    println!("----------------------------------");
    println!("This test is designed to verify that both backend and CLI handle locked accounts properly.");
    println!("Steps in this test:");
    println!("1. Backend rejects login attempts for accounts with account_status = 'locked'");
    println!("2. CLI returns a user-friendly error when attempting to log in with a locked account");
