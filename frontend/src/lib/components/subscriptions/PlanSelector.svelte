...
<div class="w-full max-w-6xl mx-auto space-y-8">
	...
	<div class="flex justify-center">
		<div class="flex items-center bg-muted rounded-lg p-1">
			<button
				class="px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 {billingCycle === 'monthly'
			...
			<button
				class="px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 relative {billingCycle === 'yearly'
					? 'bg-background text-foreground shadow-sm'
					: 'text-muted-foreground hover:text-foreground'}"
			...
			>
				Yearly
				<Badge variant="secondary" class="absolute -top-2 -right-2 text-xs px-1.5 py-0.5">
					Save 17%
				</Badge>
			</button>
		</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
		{#each plans as plan (plan.plan_type)}
			...
			<div class="relative">
				...
				<Card class="relative h-full {recommended && billingCycle === 'yearly'
