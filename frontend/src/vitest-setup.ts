...
vi.mock('$lib/api', async (importActual) => {
  ...
  return {
    ...
    apiClient: {
      ...actual.apiClient,
      getChatSessionSettings: vi.fn(() => Promise.resolve(ok({
        temperature: 1.0,
      ...
      getLorebook: vi.fn((id: string) => Promise.resolve(ok({ id, name: `Mock Lorebook ${id.substring(0, 4)}` }))),
      getChatSession: vi.fn(() => Promise.resolve(ok({
        id: 'chat-123',
        title: 'Mock Chat',
      ...
      getCharacter: vi.fn(() => Promise.resolve(ok({
        id: 'char-456',
        name: 'Mock Character',
        user_id: 'user-789',
        created_at: '2023-01-01T00:00:00Z',
