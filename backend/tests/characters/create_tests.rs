...
async fn test_create_character_minimal_fields() -> Result<(), anyhow::Error> {
    ...
    tracing::info!("Minimal Fields - Response status: {}, body: {}", status, body_text);
    
    assert_eq!(status, StatusCode::CREATED, "Character creation with minimal fields failed. Body: {}", body_text);
    ...
    assert_eq!(created_char.description.as_deref(), Some("A test character created manually"));
    assert_eq!(created_char.first_mes.as_deref(), Some("Hello, I'm a manually created test character!"));
    ...
    assert_eq!(created_char.personality.as_deref(), Some(""), "Personality should default to Some(\"\")");
    assert_eq!(created_char.scenario.as_deref(), Some(""), "Scenario should default to Some(\"\")");
    assert_eq!(created_char.mes_example.as_deref(), Some(""), "MesExample should default to Some(\"\")");
    assert_eq!(created_char.creator_notes.as_deref(), Some(""), "CreatorNotes should default to Some(\"\")");
    assert_eq!(created_char.system_prompt.as_deref(), Some(""), "SystemPrompt should default to Some(\"\")");
    assert_eq!(created_char.post_history_instructions.as_deref(), Some(""), "PostHistoryInstructions should default to Some(\"\")");
    
    assert_eq!(created_char.tags, Some(vec![]), "Tags should default to Some(vec![])");
    assert_eq!(created_char.alternate_greetings, Some(vec![]), "AlternateGreetings should default to Some(vec![])");
    
    // Assert extensions default
    assert_eq!(created_char.extensions, Some(DieselJson(JsonValue::Object(serde_json::Map::new()))), "Extensions should default to an empty JSON object");


    assert_eq!(created_char.user_id, user.id);
    assert!(!created_char.id.is_nil(), "Character ID should be a valid UUID");
    
    let now = chrono::Utc::now();
    assert!(created_char.created_at <= now && created_char.created_at > now - chrono::Duration::seconds(5), "created_at is not recent");
    assert!(created_char.updated_at <= now && created_char.updated_at > now - chrono::Duration::seconds(5), "updated_at is not recent");
...
async fn test_create_character_all_fields() -> Result<(), anyhow::Error> {
    ...
    let (status, body_text) = get_text_body(create_response).await?;
    tracing::info!("All Fields - Response status: {}, body: {}", status, body_text);
    
    assert_eq!(status, StatusCode::CREATED, "Character creation with all fields failed. Body: {}", body_text);
    
    let created_char: CharacterDataForClient = serde_json::from_str(&body_text)
        .expect("Failed to deserialize response into CharacterDataForClient");

    assert_eq!(created_char.name, "Complete Test Character");
    assert_eq!(created_char.description.as_deref(), Some("A test character with all fields populated"));
    assert_eq!(created_char.first_mes.as_deref(), Some("Hello, I'm a complete test character with all fields!"));
    assert_eq!(created_char.personality.as_deref(), Some("Friendly, helpful, and detailed"));
    assert_eq!(created_char.scenario.as_deref(), Some("In a test environment responding to API requests"));
    assert_eq!(created_char.mes_example.as_deref(), Some("User: How are you?\nCharacter: I'm doing great, thanks for asking!"));
    assert_eq!(created_char.creator_notes.as_deref(), Some("This character was created for testing the manual creation API"));
    assert_eq!(created_char.system_prompt.as_deref(), Some("You are a test character created via the API"));
    assert_eq!(created_char.post_history_instructions.as_deref(), Some("Continue being helpful in your responses"));
    
    assert_eq!(created_char.tags, Some(vec![Some("test".to_string()), Some("api".to_string()), Some("manual_creation".to_string())]));
    assert_eq!(created_char.creator.as_deref(), Some("API Tester"));
    assert_eq!(created_char.character_version.as_deref(), Some("1.0.0"));
    assert_eq!(created_char.alternate_greetings, Some(vec![Some("Hey there!".to_string()), Some("Greetings, tester!".to_string())]));
    
    assert_eq!(created_char.spec, "chara_card_v3");
    assert_eq!(created_char.spec_version, "3.0");

    let expected_extensions_json_value = json!({"custom_field": "custom_value", "nested": {"value": 123}});
    assert_eq!(created_char.extensions, Some(DieselJson(expected_extensions_json_value)));
...
async fn test_create_character_missing_required_fields() -> Result<(), anyhow::Error> {
    ...
    let test_cases = vec![
        ...
        (json!({}), "name is required, description is required, first_mes is required"),
    ];
    ...
    for (i, (test_case_json, expected_error_substring)) in test_cases.iter().enumerate() {
        ...
        let (status, body_text) = get_text_body(create_response).await?;
        tracing::info!("Missing Fields Test Case {}: Response status: {}, body: {}", i, status, body_text);
        
        assert_eq!(status, StatusCode::BAD_REQUEST,
            "Character creation with missing/empty required fields (case {}) should fail with 400 Bad Request. Body: {}", i, body_text);
        ...
        assert!(body_text.contains(expected_error_substring),
            "Error message for case {} did not contain expected substring '{}'. Full error: {}", i, expected_error_substring, body_text);
    }
