...
mod chat_overrides_api_tests {
    ...
    use uuid::Uuid; // Using reqwest for actual http client
    ...
    async fn create_chat_session_via_api(
        ...
        let response_body_bytes = response
            .bytes()
            .await
            .expect("Failed to get response bytes for create_session");
        ...
        serde_json::from_slice(&response_body_bytes)
            .expect("Failed to deserialize chat session from API response")
    }
    ...
    async fn setup_test_environment(
        ...
        let chat_session =
            create_chat_session_via_api(test_app, user.id, character.id, &session_cookie_str).await;
    ...
    async fn test_create_new_override_success() {
        ...
        let override_response: serde_json::Value = response
            .json()
            .await
            .expect("Failed to parse json response");
        assert_eq!(
            override_response["field_name"].as_str().unwrap(),
            "description"
        );
        assert!(override_response["id"].as_str().is_some());
        assert_eq!(
            override_response["chat_session_id"].as_str().unwrap(),
            chat_session.id.to_string()
        );
        assert_eq!(
    ...
    async fn test_update_existing_override_success() {
        ...
        assert_eq!(
            ...
            "Initial override creation failed"
        );
        let created_override: serde_json::Value = response1
            .json()
            .await
            .expect("Failed to parse initial override response");
        let created_at_initial = created_override["created_at"]
            .as_str()
            .expect("created_at not found or not a string")
            .to_string();
        let updated_at_initial = created_override["updated_at"]
            .as_str()
            .expect("updated_at not found or not a string")
            .to_string();
        ...
        let updated_override_response: serde_json::Value = response2
            .json()
            .await
            .expect("Failed to parse updated override response");

        assert_eq!(
            updated_override_response["field_name"].as_str().unwrap(),
            "description"
        );
        assert_eq!(
            updated_override_response["id"].as_str().unwrap(),
            created_override["id"].as_str().unwrap(),
            "ID should be the same on update"
        );
        assert_eq!(
            updated_override_response["chat_session_id"]
                .as_str()
                .unwrap(),
            chat_session.id.to_string()
        );
        assert_eq!(
            updated_override_response["original_character_id"]
                .as_str()
                .unwrap(),
        ...
        assert_eq!(
            updated_override_response["created_at"].as_str().unwrap(),
            created_at_initial,
            "created_at should not change on update"
        );
        assert_ne!(
            updated_override_response["updated_at"].as_str().unwrap(),
            updated_at_initial,
    ...
    async fn test_create_override_for_non_existent_chat_session() {
        let test_app = test_helpers::spawn_app(true, true, true).await;
        let (_guard, _user, _character, _chat_session, session_cookie_str) =
            setup_test_environment(&test_app).await;
    ...
    async fn test_create_override_response_has_message_field() {
        ...
        assert!(
            response_json.get("field_name").is_some(),
            "Response should include a 'field_name' field"
        );
        assert!(
            response_json.get("session_id").is_some()
                || response_json.get("chat_session_id").is_some(),
