...
impl SecureLlmService {
    ...
    pub async fn secure_exec_chat(
        ...
        let response = self
            .ai_client
            .exec_chat("default", request.clone(), None)
            .await
            .map_err(|e| {
        ...
        #[cfg(feature = "local-llm")]
        if let Some(ref audit_logger) = self.app_state.security_audit_logger {
            let response_length = validated_response
    ...
    pub async fn secure_stream_chat(
        ...
        #[cfg(feature = "local-llm")]
        {
            ...
            self.sanitize_prompts(&mut request.messages, user_id)
                .await?;
            self.check_resource_limits(&request, user_id).await?;
        }
        ...
        let stream = self
            .ai_client
            .stream_chat("default", request.clone(), None)
            .await
            .map_err(|e| {
...
impl AiClient for SecureLlmService {
    async fn exec_chat(
        ...
        warn!(
            "SecureLlmService::exec_chat called without user context - falling back to underlying client"
        );
    ...
    async fn stream_chat(
        ...
        warn!(
            "SecureLlmService::stream_chat called without user context - falling back to underlying client"
        );
