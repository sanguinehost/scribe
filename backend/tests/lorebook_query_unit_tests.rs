use scribe_backend::services::character_generation::{field_generator::FieldGenerator, types::*};
use std::sync::Arc;
use uuid::Uuid;
...
fn test_field_specific_keywords() {
    ...
    for test_case in test_cases {
        ...
        println!(
            "Field {:?} should use keywords: {}",
            test_case.field, test_case.expected_keywords
        );
    }
...
fn test_lorebook_context_formatting() {
    ...
    let mock_entries = vec![
        ...
        MockLorebookEntry {
            title: None,
            content: "The world is filled with magical creatures and ancient artifacts."
                .to_string(),
        },
    ];
    ...
    let expected_formatted = vec![
        "- **Character Background**: This character has a mysterious past involving ancient magic."
            .to_string(),
...
fn test_multiple_lorebook_handling() {
    ...
    let lorebook_ids = vec![Uuid::new_v4(), Uuid::new_v4(), Uuid::new_v4()];
