...
async fn test_agentic_tools_basic_functionality() {
    ...
    match triage_result {
        Ok(result) => {
            ...
            println!(
                "    ✅ Triage tool working - significance: {}",
...
async fn test_chronicle_event_creation_tool() {
    ...
    let create_event_tool =
        CreateChronicleEventTool::new(Arc::new(ChronicleService::new(test_app.db_pool.clone())));
    ...
    match create_result {
        ...
        Err(e) => {
            println!(
                "    ⚠️  Event creation failed (expected for missing user/chronicle): {}",
                e
            );
        ...
        Ok(result) => {
            ...
            if is_significant {
                ...
                if let Ok(concepts_result) = concepts_tool.execute(&messages_for_ai).await {
                    if let Some(concepts) =
                        concepts_result.get("concepts").and_then(|v| v.as_array())
                    {
...
async fn test_tool_registry_integration() {
    ...
    let create_event_tool = Arc::new(CreateChronicleEventTool::new(Arc::new(
        ChronicleService::new(test_app.db_pool.clone()),
    )));
    ...
    assert!(
        tool_names.len() >= 5,
        "Should have registered at least 5 tools"
    );
