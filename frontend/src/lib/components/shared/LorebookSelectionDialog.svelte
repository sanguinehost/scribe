	...
	async function saveChanges() {
		...
		try {
			...
			for (const lorebook of lorebooks) {
				...
				const existingAssociationsForLorebook = currentAssociations.filter(
					(a) => a.lorebook_id === lorebook.id
				);
				const existingChatAssociation = existingAssociationsForLorebook.find(
					(a) => a.source === 'Chat'
				);
				const existingCharacterAssociation = existingAssociationsForLorebook.find(
					(a) => a.source === 'Character'
				);

				// Determine the current effective state of the character lorebook (active or disabled by override)
				const isCharacterLorebookEffectivelyActive =
					existingCharacterAssociation &&
					!(
						existingCharacterAssociation.is_overridden &&
						existingCharacterAssociation.override_action === 'disable'
					);
				const isCharacterLorebookEffectivelyDisabled =
					existingCharacterAssociation &&
					existingCharacterAssociation.is_overridden &&
					existingCharacterAssociation.override_action === 'disable';
...
<Dialog bind:open onOpenChange={handleClose}>
	<DialogContent class="flex max-h-[80vh] flex-col sm:max-w-[600px]">
		<DialogHeader>
			...
			<DialogDescription>
				Select which lorebooks should be active for this chat. Associated lorebooks will provide
				...
				{:else}
					<div class="space-y-3 p-4">
						{#each filteredLorebooks as lorebook (lorebook.id)}
							<div
								class="flex items-start space-x-3 rounded-md p-2 transition-colors hover:bg-muted/50"
							>
								<input
									type="checkbox"
								...
								<div class="min-w-0 flex-1">
									<div class="truncate font-medium">{lorebook.name}</div>
