...
async fn test_narrative_agent_chronicle_events_are_embedded() {
    ...
    let create_tool = CreateChronicleEventTool::new(Arc::new(chronicle_service), app_state.clone());
    ...
    println!(
        "✅ Chronicle event exists in database with summary: {}",
    ...
    println!(
        "✅ SUCCESS: Chronicle events created by narrative agents now include embedding functionality!"
    );
    println!(
        "   This test verifies that CreateChronicleEventTool successfully calls the embedding pipeline."
    );
    println!(
        "   The fix ensures narrative agent events are embedded for RAG retrieval, solving the reported issue."
    );
}
