...
use base64::prelude::*;
use genai::chat::{ChatMessage, ChatRequest, ChatResponse, MessageContent};
...
impl LlmEncryptionService {
    ...
    pub fn new(user_id: crate::db::DbId, session_dek: SessionDek) -> Self {
    ...
    pub fn encrypt_chat_request(
        &self,
        request: &ChatRequest,
    ) -> Result<EncryptedLlmData, LlmEncryptionError> {
        ...
        let request_json = serde_json::to_string(request).map_err(|e| {
            LlmEncryptionError::EncryptionFailed(format!("JSON serialization failed: {}", e))
        })?;
    ...
    pub fn decrypt_chat_request(
        &self,
        encrypted_data: &EncryptedLlmData,
    ) -> Result<ChatRequest, LlmEncryptionError> {
        ...
        if encrypted_data.data_type != LlmDataType::ChatRequest {
            return Err(LlmEncryptionError::InvalidFormat(format!(
                "Expected ChatRequest, found {:?}",
                encrypted_data.data_type
            )));
        }
        ...
        let request_json = String::from_utf8(decrypted_bytes).map_err(|e| {
            LlmEncryptionError::DecryptionFailed(format!("UTF-8 decode failed: {}", e))
        })?;

        let chat_request: ChatRequest = serde_json::from_str(&request_json).map_err(|e| {
            LlmEncryptionError::DecryptionFailed(format!("JSON deserialization failed: {}", e))
        })?;
    ...
    pub fn decrypt_chat_response(
        &self,
        encrypted_data: &EncryptedLlmData,
    ) -> Result<ChatResponse, LlmEncryptionError> {
        ...
        // Deserialize from JSON
        let response_json = String::from_utf8(decrypted_bytes).map_err(|e| {
            LlmEncryptionError::DecryptionFailed(format!("UTF-8 decode failed: {}", e))
        })?;

        let chat_response: ChatResponse = serde_json::from_str(&response_json).map_err(|e| {
            LlmEncryptionError::DecryptionFailed(format!("JSON deserialization failed: {}", e))
        })?;
    ...
    pub fn encrypt_message_content(
        ...
        debug!(
            "Encrypting message content of type {:?} for user: {}",
            data_type, self.user_id
        );
    ...
    pub fn decrypt_message_content(
        &self,
        encrypted_data: &EncryptedLlmData,
    ) -> Result<String, LlmEncryptionError> {
        debug!(
            "Decrypting message content of type {:?} for user: {}",
            encrypted_data.data_type, self.user_id
        );
        ...
        // Convert to string
        let content = String::from_utf8(decrypted_bytes).map_err(|e| {
            LlmEncryptionError::DecryptionFailed(format!("UTF-8 decode failed: {}", e))
        })?;
    ...
    pub fn encrypt_messages_in_place(
        &self,
        messages: &mut Vec<ChatMessage>,
    ) -> Result<(), LlmEncryptionError> {
        debug!(
            "Encrypting {} messages in place for user: {}",
            messages.len(),
            self.user_id
        );
        ...
        for message in messages.iter_mut() {
            match &mut message.content {
                MessageContent::Text(text) => {
                    ...
                    let encrypted_data =
                        self.encrypt_message_content(text, LlmDataType::UserPrompt)?;
                    ...
                    *text = format!(
                        ...
                        base64::prelude::BASE64_STANDARD.encode(
                            &encrypted_data.encrypted_content
                                [..16.min(encrypted_data.encrypted_content.len())]
                        )
                    );
                }
                ...
                _ => {
                    warn!(
                        "Unsupported message content type for encryption: {:?}",
                        message.content
                    );
                }
            }
    ...
    pub fn decrypt_messages_in_place(
        &self,
        messages: &mut Vec<ChatMessage>,
    ) -> Result<(), LlmEncryptionError> {
        debug!(
            "Decrypting {} messages in place for user: {}",
            messages.len(),
            self.user_id
        );
            ...
            match &mut message.content {
                MessageContent::Text(text) => {
                    if text.starts_with("[ENCRYPTED:") {
                        warn!(
                            "Found encrypted message marker - real implementation would decrypt from storage"
                        );
    ...
    pub fn user_id(&self) -> crate::db::DbId {
        self.user_id
    }
}
...
impl StreamingEncryptionHandler {
    ...
    pub fn finalize(&self) -> Result<EncryptedLlmData, LlmEncryptionError> {
        debug!(
            "Finalizing streaming encryption with {} characters",
            self.accumulated_content.len()
        );

        self.encryption_service
            .encrypt_message_content(&self.accumulated_content, LlmDataType::AssistantResponse)
    }
...
mod tests {
    ...
    fn test_encrypted_llm_data_serialization() {
        ...
        assert_eq!(
            encrypted_data.encrypted_content,
            deserialized.encrypted_content
        );
