...
fn generate_verification_token() -> String {
    ...
    format!("{}", scribe_core::DbId::new().simple())
}
...
pub async fn create_user_with_verification(
    ...
    if let Err(e) = email_service
        .send_verification_email(user.email.as_deref().unwrap_or_default(), &user.username, &verification_token)
        .await
    {
...
pub fn create_user_sync(
    ...
    let plaintext_dek_bytes = scribe_crypto::generate_dek() // Use generate_dek instead of generate_random_bytes
    ...
    let kek_salt = scribe_crypto::generate_salt().map_err(AuthError::CryptoOperationFailed)?;

    // b. Derive the KEK using Argon2
    let kek = scribe_crypto::derive_kek(&credentials.password, &kek_salt)
        .map_err(AuthError::CryptoOperationFailed)?;
    ...
    let (encrypted_dek, dek_nonce) = scribe_crypto::encrypt_gcm(plaintext_dek_bytes.expose_secret(), &kek)
        .map_err(AuthError::CryptoOperationFailed)?;
    ...
    if recovery_phrase_value.is_none() {
        ...
        let recovery_salt = scribe_crypto::generate_salt().map_err(AuthError::CryptoOperationFailed)?;
    ...
    let (encrypted_dek_by_recovery, recovery_kek_salt, recovery_dek_nonce) = {
        ...
        let recovery_kek_salt =
            scribe_crypto::generate_salt().map_err(AuthError::CryptoOperationFailed)?;
        ...
        let recovery_key = scribe_crypto::derive_kek(&recovery_secret, &recovery_kek_salt)
            .map_err(AuthError::CryptoOperationFailed)?;
        let (encrypted_dek_by_recovery, recovery_dek_nonce) =
            scribe_crypto::encrypt_gcm(plaintext_dek_bytes.expose_secret(), &recovery_key)
                .map_err(AuthError::CryptoOperationFailed)?;
        (
    ...
    let new_user = NewUser {
        ...
        email: Some(credentials.email.clone()),       // Clone email from credentials
        kek_salt,
...
pub fn verify_credentials(
    ...
    if is_valid {
        ...
        let kek = scribe_crypto::derive_kek(&password, &user.kek_salt) // user.kek_salt should be a &str
            .map_err(|e| {
        ...
        // scribe_crypto::decrypt_gcm returns Result<SecretBox<Vec<u8>>, CryptoError>
        let decrypted_dek_secret_box = scribe_crypto::decrypt_gcm(
            &user.encrypted_dek,
            &user.dek_nonce,
...
pub async fn change_user_password(
    ...
    let old_kek = scribe_crypto::derive_kek(&current_password_payload, &current_db_user.kek_salt)
        .map_err(|e| {
    ...
    let plaintext_dek_secret = scribe_crypto::decrypt_gcm(
    ...
    let new_kek_salt_str = scribe_crypto::generate_salt().map_err(AuthError::CryptoOperationFailed)?;

    // 5. Derive new KEK from new_password and new_kek_salt
    debug!("Deriving new KEK from new password...");
    let new_kek = scribe_crypto::derive_kek(&new_password_payload, &new_kek_salt_str).map_err(|e| {
    ...
    let (new_ciphertext_dek_bytes, new_nonce_dek_bytes) =
        scribe_crypto::encrypt_gcm(plaintext_dek_secret.expose_secret(), &new_kek).map_err(|e| {
...
pub async fn recover_user_password_with_phrase(
    backend: &user_store::Backend,
    pool: &crate::db::DbPool, // Added pool for direct DB interaction if needed, or pass to backend methods
    identifier: String,
    ...
    let rkek = scribe_crypto::derive_kek(&recovery_phrase_payload, recovery_kek_salt).map_err(|e| {
    ...
    let plaintext_dek_secret =
        scribe_crypto::decrypt_gcm(encrypted_dek_by_recovery, recovery_dek_nonce_bytes, &rkek)
            .map_err(|e| {
