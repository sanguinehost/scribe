#[cfg(test)]
mod tests {
    use scribe_backend::services::character_parser::{parse_character_card_png, ParsedCharacterCard};
    ...
    let png_path = "/home/socol/Downloads/main_my-hero-academia-rpg-overhaul-5ca1a8ee2ac5_spec_v2.png";
    ...
    match parsed {
        Ok(ParsedCharacterCard::V2Fallback(data)) => {
            ...
            assert!(data.character_book.is_some(), "character_book should be Some");
            let book = data.character_book.unwrap();
            println!("Character book found: {:?}", book);
        }
        Ok(ParsedCharacterCard::V3(card)) => {
            println!("Parsed as V3");
            assert!(card.data.character_book.is_some(), "character_book should be Some");
