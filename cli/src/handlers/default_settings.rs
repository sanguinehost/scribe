...
impl DefaultSettings {
    pub fn to_update_request(&self) -> UpdateChatSettingsRequest {
        UpdateChatSettingsRequest {
            ...
            temperature: self.temperature.as_ref().map(|s| BigDecimal::parse_bytes(s.as_bytes(), 10).unwrap_or_else(|| BigDecimal::from(0))), // Convert string to BigDecimal
...
pub fn load_default_settings() -> DefaultSettings {
    ...
    match fs::read_to_string(&settings_path) {
        Ok(contents) => {
            match serde_json::from_str::<DefaultSettings>(&contents) {
...
pub async fn handle_default_settings_action<H: IoHandler, C: HttpClient>(
    ...
    io_handler.write_line(&format!("  Thinking Budget: {}", settings.gemini_thinking_budget.unwrap_or(0)))?;
    io_handler.write_line(&format!("  Enable Code Execution: {}", settings.gemini_enable_code_execution.unwrap_or(false)))?;
    io_handler.write_line(&format!("  System Prompt: {}", settings.system_prompt.as_deref().unwrap_or("None")))?;
    io_handler.write_line(&format!("  Temperature: {}", settings.temperature.as_deref().unwrap_or("0.0")))?;
    io_handler.write_line(&format!("  Max Output Tokens: {}", settings.max_output_tokens.unwrap_or(0)))?;
    ...
    match choice.as_str() {
        "1" => {
            ...
            if let Ok(budget) = input.trim().parse::<i32>() {
                ...
                io_handler.write_line(&format!("Thinking budget updated to: {:?}", settings.gemini_thinking_budget))?;
            } else {
                io_handler.write_line("Invalid input. Thinking budget not updated.")?;
            }
        }
        "2" => {
            let input = io_handler.read_line("Enable code execution? (y/n): ")?.trim().to_lowercase();
        ...
        "3" => {
            ...
            io_handler.write_line(&format!("System prompt updated to: {:?}", settings.system_prompt))?;
        }
        "4" => {
            ...
            if let Ok(temp) = input.trim().parse::<f32>() {
                if temp >= 0.0 && temp <= 1.0 {
                    ...
                    io_handler.write_line("Temperature must be between 0.0 and 1.0. Not updated.")?;
                }
            } else {
        ...
        "5" => {
            ...
            if let Ok(tokens) = input.trim().parse::<i32>() {
                ...
                io_handler.write_line(&format!("Max output tokens updated to: {:?}", settings.max_output_tokens))?;
            } else {
    ...
    let save_choice = io_handler.read_line("Save these settings as default? (y/n): ")?.trim().to_lowercase();
...
pub async fn apply_default_settings_to_session<C: HttpClient>(
    ...
    client.update_chat_settings(session_id, &update_request).await?;
