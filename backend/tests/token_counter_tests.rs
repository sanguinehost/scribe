...
use scribe_backend::services::hybrid_token_counter::{HybridTokenCounter, CountingMode};
...
async fn test_hybrid_token_counter_local() {
    ...
    let token_count = counter.count_tokens(text, CountingMode::LocalOnly, None)
        .await
    ...
    let messages = vec![
        ...
        ("model".to_string(), "I'm doing well, thank you for asking!".to_string()),
        ("user".to_string(), "Tell me about token counting.".to_string()),
    ];
    
    let chat_token_count = counter.count_tokens_chat(&messages, CountingMode::LocalOnly, None)
        .await
    ...
    let multimodal_token_count = counter.count_tokens_multimodal(
        text,
        None, // No images
        Some(video_duration),
        Some(audio_duration),
    ...
    assert_eq!(multimodal_token_count.video, (video_duration * 263.0) as usize);
    
    // Verify audio token count (32 tokens per second)
    assert_eq!(multimodal_token_count.audio, (audio_duration * 32.0) as usize);
...
async fn test_hybrid_token_counter_api() {
    ...
    let test_texts = vec![
        ...
        ("This is a longer text sample that contains multiple sentences. It has punctuation, numbers like 1234, and should test the tokenizer more thoroughly. How well does it handle this?", "Longer English text"),
    ...
    for (text, description) in &test_texts {
        ...
        // Local counting
        let local_count = counter.count_tokens(text, CountingMode::LocalOnly, None)
            .await
            .expect("Failed to count tokens locally");
        
        // API counting
        let api_count = counter.count_tokens(text, CountingMode::ApiOnly, None)
            .await
            .expect("Failed to count tokens with API");
        
        // Hybrid counting (prefer API)
        let hybrid_count = counter.count_tokens(text, CountingMode::HybridPreferApi, None)
            .await
            .expect("Failed to count tokens with hybrid approach");
        
        // Print the difference percentage
        let difference_percent = ((api_count.total as f64 - local_count.total as f64).abs() / local_count.total as f64) * 100.0;
        ...
        println!("- Tokens per character (API): {:.2}", api_count.total as f64 / text.len() as f64);
        println!("- Tokens per character (Local): {:.2}", local_count.total as f64 / text.len() as f64);
    ...
    let chat_token_count = counter.count_tokens_chat(&messages, CountingMode::HybridPreferApi, None)
        .await
        .expect("Failed to count chat tokens");
