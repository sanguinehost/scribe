...
mod payment_integration_tests {
    use scribe_backend::{
        ...
        services::payment::paddle_service::{
            PaddleService, PaddleEventType, PaddleWebhook, 
            CreateTransactionRequest, CreateTransactionResponse,
            TransactionItem, TransactionCheckout
        }
    };
    ...
    fn test_webhook_payload_parsing_all_event_types() {
        ...
        for (event_name, expected_type) in event_types {
            ...
            let webhook = service.parse_webhook_payload(payload.to_string().as_bytes());
            assert!(webhook.is_ok(), "Event type {} should parse successfully", event_name);
            
            let webhook = webhook.unwrap();
            assert_eq!(webhook.event_type, expected_type, "Event type should match for {}", event_name);
        }
    }
    ...
    async fn test_create_customer_sandbox() {
        ...
        match result {
            Ok(customer) => {
                ...
                assert_eq!(customer.name, test_name.map(String::from), "Name should match");
                println!("✓ Successfully created customer: {}", customer.id);
            },
            Err(e) => {
                // Log the error but don't fail the test in case of API issues
                println!("⚠ Customer creation failed (this may be due to API limits or network issues): {}", e);
            }
        }
    }
    ...
    async fn test_create_transaction_sandbox() {
        ...
        let customer_result = service.create_customer(&test_email, Some("Transaction Test User")).await;
        
        match customer_result {
            Ok(customer) => {
                println!("✅ Created customer: {} with email: {}", customer.id, test_email);
                ...
                match result {
                    Ok(transaction) => {
                        assert!(!transaction.transaction_id.is_empty(), "Transaction ID should not be empty");
                        assert!(!transaction.checkout_url.is_empty(), "Checkout URL should not be empty");
                        
                        println!("✅ Successfully created transaction: {}", transaction.transaction_id);
                        println!("✅ Transaction status: {}", transaction.status);
                        println!("🔗 **CHECKOUT URL FOR YOU TO COMPLETE:**");
                        println!("{}", transaction.checkout_url);
                        println!("📝 Open this URL in your browser to complete the transaction in Paddle sandbox");
                        ...
                        if transaction.checkout_url.contains("?_ptxn=") {
                            ...
                            println!("⚠ Warning: Checkout URL may not contain expected ?_ptxn= parameter");
                        }
                    },
                    Err(e) => {
            ...
            Err(e) => {
                println!("⚠ Cannot test transaction creation without customer creation: {}", e);
            }
        }
    }
    ...
    async fn test_get_transaction_checkout_url_for_real_customer() {
        ...
        let customer_result = service.create_customer(&test_email, Some("Checkout Test User")).await;
                ...
                match result {
                    Ok(transaction) => {
                        ...
                        if transaction.checkout_url.contains("?_ptxn=") {
                            ...
                            let transaction_id_from_url = transaction.checkout_url
                                .split("?_ptxn=")
                                .nth(1)
                                .unwrap_or("not_found");
                            println!("✅ Transaction ID from URL: {}", transaction_id_from_url);
                        } else if transaction.checkout_url.contains(&transaction.transaction_id) {
                            println!("✅ Checkout URL contains transaction ID in some form");
                        } else {
                            println!("⚠ Warning: Checkout URL format may be unexpected");
                            println!("   Expected to contain transaction ID: {}", transaction.transaction_id);
                        }
                    },
                    Err(e) => {
                        println!("⚠ Transaction creation failed: {}", e);
    ...
    async fn test_get_subscription_sandbox() {
        ...
        assert!(result.is_err(), "Getting non-existent subscription should fail");
    ...
    fn test_configuration_loading() {
        ...
        assert!(config.paddle_api_key.is_some(), "Paddle API key should be loaded from environment");
        assert!(config.paddle_webhook_secret.is_some(), "Webhook secret should be loaded from environment");
        assert!(config.paddle_sandbox_mode, "Should be in sandbox mode for tests");
        assert_eq!(config.free_tier_token_limit, 50000);
        assert_eq!(config.grace_period_days, 7);
        assert!(!config.enforce_limits, "Should not enforce limits in development");
    }
}
