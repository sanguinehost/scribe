...
async fn test_create_chat_session_character_other_user() -> anyhow::Result<()> {
    ...
    let login_request_user1 = Request::builder()
        ...
        .uri("/api/auth/login")
        .header(header::CONTENT_TYPE, mime::APPLICATION_JSON.as_ref())
        .body(Body::from(serde_json::to_string(&login_payload_user1).unwrap()))
        .unwrap();
    let login_response_user1 = test_app.router.clone().oneshot(login_request_user1).await.unwrap();
    ...
    let character_conn_obj = test_app.db_pool.get().await.expect("Failed to get DB connection for character creation U1");
    let character_user1: DbCharacter = character_conn_obj.interact(move |actual_pg_conn| {
        let new_character_values = NewCharacter {
        ...
        diesel::insert_into(characters::table)
            .values(&new_character_values)
            .get_result::<DbCharacter>(actual_pg_conn)
    }).await.map(|result| result.expect("Error saving character for User 1")).expect("Interact join error U1");
    ...
    let login_request_user2 = Request::builder()
        ...
        .uri("/api/auth/login")
        .header(header::CONTENT_TYPE, mime::APPLICATION_JSON.as_ref())
        .body(Body::from(serde_json::to_string(&login_payload_user2).unwrap()))
        .unwrap();
    let login_response_user2 = test_app.router.clone().oneshot(login_request_user2).await.unwrap();
...
async fn create_chat_session_character_not_owned_integration() -> anyhow::Result<()> {
    ...
    let user1_id_clone = user1.id;
    let character_conn_obj = test_app.db_pool.get().await.expect("Failed to get DB connection for character creation user1 integ");
    let character_user1: DbCharacter = character_conn_obj.interact(move |actual_pg_conn| {
        let new_character_values = NewCharacter {
            user_id: user1_id_clone, 
            spec: "character_card_v3_example".to_string(),
        ...
        diesel::insert_into(characters::table)
            .values(&new_character_values)
            .get_result::<DbCharacter>(actual_pg_conn)
    }).await.map(|result| result.expect("Error saving character user1 integ")).expect("Interact join error user1 integ");
    ...
    let request = Request::builder()
        .method(Method::GET)
        .uri(format!("/api/chats-api/chats/{}", character_user1.id)) // Corrected URI
...
async fn test_get_chat_session_details_unauthorized() {
    ...
    let character_conn_obj = test_app.db_pool.get().await.expect("Failed to get DB conn for char unauth");
    let character: DbCharacter = character_conn_obj.interact(move |actual_pg_conn| {
    ...
    let request = Request::builder()
        .method(Method::GET)
        .uri(format!("/api/chats-api/chats/{}", session_id_clone)) // Corrected URI
        // No auth cookie
        .body(Body::empty())
        .unwrap();
...
async fn test_get_chat_session_details_invalid_uuid() {
    ...
    let request = Request::builder()
        .method(Method::GET)
        .uri(format!("/api/chats-api/chats/{}", invalid_session_id)) // Corrected URI, Invalid UUID in path
        .header(header::COOKIE, auth_cookie)
        .body(Body::empty())
        .unwrap();
...
async fn test_create_session_saves_first_mes() -> Result<(), AnyhowError> {
    ...
    rng.try_fill_bytes(&mut dek_bytes).expect("Failed to fill bytes for DEK");
