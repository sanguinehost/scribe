<script lang="ts">
	...
	const fallbackOrder = $derived(() => {
		...
		return {
			...
			total_amount: amount,
			currency: 'USD',
			next_billing_date: new Date(Date.now() + (period === 'monthly' ? 30 : 365) * 24 * 60 * 60 * 1000).toISOString(),
			savings_message: isYearly ? planFeatures.billing_features?.yearly?.savings_message : undefined,
