...
use scribe_backend::{
    ...
    test_helpers::{TestApp, TestAppGuard, TestDataGuard, spawn_app_permissive_rate_limiting},
};
...
fn create_duplicate_everest_messages(
    user_id: Uuid,
    session_id: Uuid,
    ...
    let messages_content = vec![
        (
            "user",
            "I descend upon Everest and wave my hand, cleansing it of accumulated human filth.",
        ),
        (
            "assistant",
            "You arrive at Mount Everest's peak. With a gesture, all human debris vanishes, leaving the mountain pristine.",
        ),
        (
            "user",
            "I use my cosmic powers to remove all pollution from Mount Everest.",
        ),
        (
            "assistant",
            "Your divine energy sweeps across Everest, dissolving tents, ropes, and waste, restoring its natural state.",
        ),
        (
            "user",
            "I cleanse Mount Everest of all the garbage left by climbers over the years.",
        ),
        (
            "assistant",
            "The mountain is purified by your will, every trace of human contamination eliminated.",
        ),
    ];
...
async fn create_existing_everest_events(
    user_id: Uuid,
    chronicle_id: Uuid,
    test_app: &TestApp,
) -> AnyhowResult<()> {
...
async fn test_search_knowledge_base_tool_functionality() {
    ...
    create_existing_everest_events(user_id, chronicle_id, &test_app)
        .await
        .unwrap();
    ...
    println!(
        "Search tool found {} results for Everest cleansing",
        results.len()
    );
...
async fn test_deduplication_failure_multiple_everest_events() {
    ...
    let duplicate_messages =
        create_duplicate_everest_messages(user_id, session_id, &session_dek).unwrap();
    ...
    println!(
        "Mount Everest related events found: {}",
        everest_events.len()
    );
    ...
    if everest_events.len() > 2 {
        println!(
            "❌ DE-DUPLICATION FAILURE: Found {} Everest events, expected 2 or fewer",
            everest_events.len()
        );
        println!(
            "This demonstrates the de-duplication bug where similar events are not being filtered out."
        );
        ...
        for i in 0..everest_events.len() {
            for j in i + 1..everest_events.len() {
                ...
                let common_terms: Vec<_> = key_terms
                    .iter()
        ...
        if !similar_pairs.is_empty() {
            println!(
                "❌ Found {} pairs of very similar events:",
                similar_pairs.len()
            );
            for (i, j, common_count) in similar_pairs {
                println!(
                    "  Event {} and {} share {} key terms",
                    i + 1,
                    j + 1,
                    common_count
                );
        ...
        println!(
            "✅ De-duplication working: Only {} Everest events found",
            everest_events.len()
        );
    }
}
...
async fn test_chronicle_context_retrieval_for_deduplication() {
    ...
    assert!(
        !events.is_empty(),
        "Should have existing events for context"
    );
    ...
    assert!(
        !everest_events.is_empty(),
        "Should have Everest events in context"
    );

    println!(
        "✅ Context retrieval test: Found {} total events, {} Everest-related",
        events.len(),
        everest_events.len()
    );
...
async fn test_ai_triage_with_existing_context() {
    ...
    let duplicate_messages =
        create_duplicate_everest_messages(user_id, Uuid::new_v4(), &session_dek).unwrap();
    ...
    let triage_tool =
        scribe_backend::services::agentic::AnalyzeTextSignificanceTool::new(mock_ai_client.clone());
    let triage_result = triage_tool.execute(&triage_params).await.unwrap();

    println!("Triage result with context: {:?}", triage_result);

    let is_significant = triage_result
        .get("is_significant")
