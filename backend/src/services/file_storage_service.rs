...
impl FileStorageService {
    ...
    pub async fn init(&self) -> Result<(), AppError> {
        ...
        fs::create_dir_all(&characters_dir)
            .await
            .map_err(|e| AppError::InternalServerErrorGeneric(format!("Failed to create upload directories: {}", e)))?;
    ...
    pub async fn save_character_avatar(
        ...
        fs::create_dir_all(&characters_dir)
            .await
            .map_err(|e| AppError::InternalServerErrorGeneric(format!("Failed to create characters directory: {}", e)))?;

        // Write the image data
        fs::write(&file_path, image_data)
            .await
            .map_err(|e| AppError::InternalServerErrorGeneric(format!("Failed to write avatar file: {}", e)))?;
    ...
    pub async fn load_character_avatar(&self, relative_path: &str) -> Result<Bytes, AppError> {
        ...
        let data = fs::read(&file_path)
            .await
            .map_err(|e| AppError::InternalServerErrorGeneric(format!("Failed to read avatar file: {}", e)))?;
    ...
    pub async fn delete_character_avatar(&self, relative_path: &str) -> Result<(), AppError> {
        ...
        if file_path.exists() {
            fs::remove_file(&file_path)
                .await
                .map_err(|e| AppError::InternalServerErrorGeneric(format!("Failed to delete avatar file: {}", e)))?;
...
mod tests {
    ...
    async fn test_character_avatar_storage() {
        ...
        let loaded_data = storage
            .load_character_avatar(&saved_path)
            .await
            .unwrap();
        ...
        // Delete avatar
        storage
            .delete_character_avatar(&saved_path)
            .await
            .unwrap();
