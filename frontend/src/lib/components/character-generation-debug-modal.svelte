...
<Dialog.Root {open} onOpenChange={handleOpenChange}>
	<Dialog.Content class="flex max-h-[85vh] max-w-5xl flex-col overflow-hidden">
		<Dialog.Header class="flex flex-row items-center justify-between border-b pb-4">
			<div>
				...
				<Dialog.Description class="mt-1 text-sm text-muted-foreground">
					Debug information from character field generation including system prompt and lorebook
					integration
				</Dialog.Description>
			</div>
		</Dialog.Header>
			...
			{:else}
				<div class="h-full space-y-4 overflow-auto">
					...
					<div class="rounded-lg border bg-muted/20">
						<div class="sticky top-0 border-b bg-muted/80 px-4 py-2 backdrop-blur-sm">
							<div class="flex items-center justify-between">
								<div
									class="flex items-center gap-2 text-xs font-medium text-blue-600 dark:text-blue-400"
								>
						...
						<div class="grid grid-cols-2 gap-4 p-4 text-sm md:grid-cols-4">
							...
							<div>
								...
								<div class="mt-1">
									{generationResponse?.metadata?.tokens_used?.toLocaleString() || 'Unknown'}
								</div>
							</div>
							<div>
					...
					{#if lorebookInfo}
						<div class="rounded-lg border bg-muted/20">
							<div class="sticky top-0 border-b bg-muted/80 px-4 py-2 backdrop-blur-sm">
								<div class="flex items-center justify-between">
									<div
										class="flex items-center gap-2 text-xs font-medium {lorebookInfo.included
											? 'text-green-600 dark:text-green-400'
											: 'text-orange-600 dark:text-orange-400'}"
									>
										<div
											class="h-2 w-2 rounded-full {lorebookInfo.included
												? 'bg-green-500'
												: 'bg-orange-500'}"
										></div>
					...
					{#if debugInfo?.system_prompt}
						<div class="rounded-lg border bg-muted/20">
							<div class="sticky top-0 border-b bg-muted/80 px-4 py-2 backdrop-blur-sm">
								<div class="flex items-center justify-between">
									<div
										class="flex items-center gap-2 text-xs font-medium text-purple-600 dark:text-purple-400"
									>
							...
							<pre
								class="max-h-64 overflow-auto whitespace-pre-wrap break-words p-4 font-mono text-xs leading-relaxed text-foreground/90">{debugInfo.system_prompt}</pre>
						</div>
					{/if}
					...
					{#if debugInfo?.user_message}
						<div class="rounded-lg border bg-muted/20">
							...
							<pre
								class="max-h-64 overflow-auto whitespace-pre-wrap break-words p-4 font-mono text-xs leading-relaxed text-foreground/90">{debugInfo.user_message}</pre>
						</div>
					{/if}
				</div>
			{/if}
