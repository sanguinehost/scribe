...
use genai::chat::{
    ChatMessage, ChatOptions, ChatRequest, ChatResponse, ChatRole, ChatStreamEvent, MessageContent,
    Usage,
};
...
impl LlamaCppClient {
    ...
    pub async fn new(config: LlamaCppConfig) -> Result<Self, LocalLlmError> {
        ...
        let security = Arc::new(SecurityControls {
            ...
            resource_limiter: std::sync::Mutex::new(ResourceLimiter::new(
        ...
        let server_manager =
            Arc::new(LlamaCppServerManager::new(config.clone(), model_manager.clone()).await?);
    ...
    async fn ensure_server_started_with_model(
        &self,
        model_name: &str,
        ...
        match server_info.state {
            ServerState::Running => {
                ...
                if let Some(current_model) = &server_info.model_loaded {
                    if current_model == model_name || current_model.contains(model_name) {
                        info!(
                            "Server already running with correct model: {}",
                            current_model
                        );
                        return Ok(());
                    } else {
                        info!(
                            "Server running with different model ({}), switching to {}",
                    ...
                    info!(
                        "Server running but no model loaded, restarting with model: {}",
                        model_name
                    );
                    self.server_manager.stop().await?;
                }
            ...
            ServerState::Error(ref error) => {
                warn!(
                    "Server in error state ({}), restarting with model: {}",
                    error, model_name
                );
            ...
            _ => {
                info!(
                    "Server in state {:?}, waiting and then starting with model: {}",
                    server_info.state, model_name
                );
        ...
        self.server_manager
            .start_with_model(Some(model_name))
            .await?;
    ...
    pub async fn switch_model(&self, model_name: &str) -> Result<(), LocalLlmError> {
        ...
        let model_available = models
            .iter()
            .any(|m| m.name == model_name && m.is_downloaded);
        if !model_available {
    ...
    fn convert_request(
        ...
        request: &ChatRequest,
        stream: bool,
    ) -> Result<LlamaCppChatRequest, LocalLlmError> {
    ...
    fn convert_response(&self, response: LlamaCppResponse) -> Result<ChatResponse, LocalLlmError> {
        let choice = response.choices.into_iter().next().ok_or_else(|| {
            LocalLlmError::ServerUnavailable("No choices in response".to_string())
        })?;
    ...
    async fn make_request(
        &self,
        llamacpp_request: LlamaCppChatRequest,
    ) -> Result<LlamaCppResponse, LocalLlmError> {
        let url = format!(
        ...
        let llamacpp_response: LlamaCppResponse = response.json().await.map_err(|e| {
            LocalLlmError::ServerUnavailable(format!("Response parsing failed: {}", e))
        })?;
    ...
    async fn make_streaming_request(
        &self,
        llamacpp_request: LlamaCppChatRequest,
    ) -> Result<ChatStream, LocalLlmError> {
        let url = format!(
            "http://{}:{}/v1/chat/completions",
            self.config.server_host, self.config.server_port
        );
        ...
        if !response.status().is_success() {
            ...
            let error_body = response
                ...
                .unwrap_or_else(|_| "Unknown error".to_string());
            error!(
                "LlamaCpp server returned error - Status: {}, Body: {}",
                status, error_body
            );
            return Err(LocalLlmError::ServerUnavailable(format!(
                "HTTP {} error: {}",
    ...
    async fn apply_security_controls(
        &self,
...
impl AiClient for LlamaCppClient {
    ...
    async fn exec_chat(
        ...
        self.ensure_server_started_with_model(model_name)
            .await
            .map_err(|e| {
                AppError::InternalServerErrorGeneric(format!(
                    "Failed to start server with model {}: {}",
                    model_name, e
                ))
            })?;
        ...
        let health_status =
            self.health_checker.check_health().await.map_err(|e| {
                LocalLlmError::ServerUnavailable(format!("Health check failed: {}", e))
            })?;
        ...
        let response = self
            .resilience
            .execute_with_retry(|| async { self.make_request(llamacpp_request.clone()).await })
            .await?;
        ...
        // Record metrics
        let prompt_tokens = llamacpp_request
            .messages
            .iter()
