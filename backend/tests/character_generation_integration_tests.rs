...
async fn test_alternate_greeting_generation() {
    ...
    assert_ne!(
        ...
        "Should not get internal server error"
    );
    ...
    if status == StatusCode::OK {
        ...
        match json_response {
            Ok(json) => {
                ...
                let mentions_scenario = content.to_lowercase().contains("security")
                    || content.to_lowercase().contains("energy")
                    || content.to_lowercase().contains("new hire");

                // Check for narrative style (dialogue, rich descriptions)
                let is_narrative_style =
                    content.contains("\"") && content.chars().filter(|&c| c == '\n').count() >= 3;

                // Check for system/game style (CURRENT STATE and INVENTORY sections)
                let is_system_style =
                    content.contains("CURRENT STATE:") && content.contains("INVENTORY");
