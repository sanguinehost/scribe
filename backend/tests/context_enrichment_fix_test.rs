...
async fn test_context_enrichment_with_encryption() -> Result<()> {
    ...
    if !results.is_empty() {
        ...
        if content.contains("[encrypted]") {
            ...
            panic!(
                "Content should be decrypted with SessionDek, got encrypted placeholder: {}",
                content
            );
        }

        println!(
            "✅ Found {} results with SessionDek - content properly decrypted!",
            results.len()
        );
    } else {
        // No results due to mock embeddings, but that's OK - the important thing is no SessionDek error
        println!(
            "⚠️  No results found due to MockEmbeddingClient vectors, but SessionDek was properly passed (no 'No SessionDek provided' error)"
        );
    }
    ...
    println!(
        "Search result without SessionDek: {:#?}",
        search_result_no_dek
    );
