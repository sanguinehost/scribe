...
pub async fn handle_chat_config_action<H: IoHandler, C: HttpClient>(
    ...
    let sessions = match client.list_chat_sessions().await {
        Ok(sessions) => {
            if sessions.is_empty() {
                io_handler.write_line("You have no chat sessions. Please create a chat session first.")?;
                return Ok(());
            }
            sessions
        }
    ...
    let session_choice = io_handler.read_line("Select a chat session to configure (enter number):")?;
    ...
    io_handler.write_line(&format!("Configuring chat session: {} (ID: {})", 
        selected_session.title.as_deref().unwrap_or("Untitled"), chat_id))?;
    ...
    let budget_str = io_handler.read_line("Set thinking budget (optional, integer, e.g. 1024, press Enter to skip):")?;
    ...
    let exec_str = io_handler.read_line("Enable code execution (optional, true/false, press Enter to skip):")?;
    ...
    let system_prompt = io_handler.read_line("Set system prompt (optional, press Enter to skip):")?;
    if !system_prompt.trim().is_empty() {
        settings_to_update.system_prompt = Some(system_prompt.trim().to_string());
    }
    
    // 8. Configure temperature
    let temp_str = io_handler.read_line("Set temperature (optional, float 0.0-2.0, press Enter to skip):")?;
