...
BEGIN
    -- Log to application logs via RAISE NOTICE
    RAISE NOTICE 'PADDLE_RECOVERY: subscription_id=%, customer_id=%, action=%, result=%',
        p_subscription_id, p_paddle_customer_id, p_action, p_result;
END;
$$ LANGUAGE plpgsql;
...
DROP FUNCTION log_paddle_recovery(UUID, VARCHAR, VARCHAR, VARCHAR);
